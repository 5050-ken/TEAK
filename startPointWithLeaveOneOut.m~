...
for datasetCounter = 1:size(allMyDatasets,2)
    ...
    for counter = 1:20
        ...
        runNumberOutOf20 = counter
    end

    % save workspace
    % eval(['save leave_one_out_results_workspaces\',char(allMyDatasets(datasetCounter)),'.mat;']);
    %  修正ver  20200504
    matfile = fullfile('leave_one_out_results_workspaces/', char(allMyDatasets(datasetCounter)));
    save(matfile)
