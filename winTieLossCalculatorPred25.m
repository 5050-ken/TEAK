...
winTieLossFile = fopen('winTieLossValuesPred25.txt','w');
...
for datasetCounter = 1:10
   ...
   eval(['load 10_by_3_way_results_workspaces\',char(allMyDatasets(datasetCounter)),'.mat;']);
    ...
    % open file to append
    winTieLossFile = fopen('winTieLossValuesPred25.txt','a');
            ...
            if i ~= j   % if i and j are different
                ...
                for k = 1:size(mre1,1)
                    ...
                    if H == 0   % if they are the same
                        ...
                        eval(['pred25i = size(find(mre2',num2str(i),'(',num2str(k),',:)<=0.25),2)/size(mre1,2);']);
                        eval(['pred25j = size(find(mre2',num2str(j),'(',num2str(k),',:)<=0.25),2)/size(mre1,2);']);
                        if pred25i > pred25j
